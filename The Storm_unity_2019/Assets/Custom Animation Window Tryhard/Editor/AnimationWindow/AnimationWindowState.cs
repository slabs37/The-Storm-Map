...
namespace UnityEditorInternal.Enemeteen {
	...
	internal class AnimationWindowState : ScriptableObject, ICurveEditorState {
		...
		[SerializeField] private NestedPrefabPlayback m_NestedPrefabPlayback;
		[SerializeField] private NestedPrefabControlsState m_NestedPrefabControlsState;
		...
		public NestedPrefabPlayback nestedPrefabPlayback => m_NestedPrefabPlayback;
		public NestedPrefabControlsState nestedPrefabControlsState => m_NestedPrefabControlsState;
		...
		public void OnEnable() {
			...
			if (m_NestedPrefabPlayback == null)
				m_NestedPrefabPlayback = new NestedPrefabPlayback();

			if (m_NestedPrefabControlsState == null)
			{
				m_NestedPrefabControlsState = new NestedPrefabControlsState();
				m_NestedPrefabControlsState.Load();
			}

			TrySetupParticleSystemPlayback();
			TrySetupNestedPrefabPlayback();
		}
		...
		private void TrySetupNestedPrefabPlayback()
		{
			// Scanned from activeRootGameObject (the object that actually owns the
			// Animator/Animation being edited) rather than activeGameObject, so nested
			// prefabs are found correctly even when a child object is selected in the
			// hierarchy panel.
			if (activeRootGameObject != null)
				m_NestedPrefabPlayback.Setup(activeRootGameObject);
		}
